...
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var splitWords = function (s) {    return A2($String.split," ",s);};
   var splitLines = function (s) {    return A2($String.split,"\n",s);};
   var main = Elm.Native.Task.make(_elm).perform(A2($Task.onError,
   A2($Task.andThen,
   A2($Task.andThen,$Process.exec("brew list --versions"),function (n) {    return $Task.succeed(A2($List.map,splitWords,splitLines(n)));}),
   $Console.log),
   $Console.log));
   return _elm.Main.values = {_op: _op,splitLines: splitLines,splitWords: splitWords};
};
